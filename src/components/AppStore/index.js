...
export default class AppStore extends Component {
  ...
  getFilteredResults = () => {
    ...
    return appsList.filter(
      eachAppObject => eachAppObject.category === activeTabId,
    )
  }

  getSearchedResults = (filteredAppList, searchInput) =>
    filteredAppList.filter(eachAppObject =>
      eachAppObject.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
  ...
  render() {
    ...
    const filteredAppList = this.getFilteredResults()

    const searchedAppListResults = this.getSearchedResults(
      filteredAppList,
      searchInput,
    )
      ...
      <div className="home-container">
        <div className="app-store-container">
          ...
          <ul className="tab-items-container">
            {tabsList.map(eachTabObject => (
              <TabItem
                changeTab={this.changeTab}
                tabDetails={eachTabObject}
                key={eachTabObject.tabId}
                isActive={eachTabObject.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="app-display-container">
            {searchedAppListResults.map(eachAppDetailsObject => (
              <AppItem
                appDetails={eachAppDetailsObject}
                key={eachAppDetailsObject.appId}
              />
            ))}
          </ul>
        </div>
